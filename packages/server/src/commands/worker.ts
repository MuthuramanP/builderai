...
export default class Worker extends BaseCommand {
    ...
    async run(): Promise<void> {
        logger.info('Starting NuAgentOne Worker...')
    ...
    async stopProcess() {
        try {
            ...
            logger.info(`Shutting down NuAgentOne Prediction Worker ${this.predictionWorkerId}...`)
            await predictionWorker.close()

            const upsertWorker = queueManager.getQueue('upsert').getWorker()
            logger.info(`Shutting down NuAgentOne Upsertion Worker ${this.upsertionWorkerId}...`)
            await upsertWorker.close()
        } catch (error) {
            logger.error('There was an error shutting down NuAgentOne Worker...', error)
            await this.failExit()
        }
